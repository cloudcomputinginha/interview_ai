...
# from langchain.embeddings import BedrockEmbeddings  # or OpenAIEmbeddings etc.
# from langchain.text_splitter import RecursiveCharacterTextSplitter
# from langchain.document_loaders import TextLoader

def estimate_tokens(text: str) -> int:
    return int(len(text) / 4)  # 평균 1 token ≈ 4 characters
...
class BedrockClient(LLMClient):
    ...
    def generate_questions(self, info, cover_letter) -> str:
        if not self.use_llm:
            return "1. 자기소개\n2. 지원동기\n3. 장점과 단점\n4. 직무 관련 경험\n5. 향후 커리어 목표"

        # loader = TextLoader("/home/ubuntu/images/image_1.txt")  # 지원자 사전 문서 등
        # docs = loader.load()
        # splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=50)
        # split_docs = splitter.split_documents(docs)

        # embeddings = BedrockEmbeddings(region_name=os.getenv("AWS_REGION", "us-east-1"))  # or your preferred embeddings
        # vectorstore = FAISS.from_documents(split_docs, embeddings)
        # retriever = vectorstore.as_retriever()

        # query = f"{info['result']['interview']['corporateName']} {info['result']['interview']['jobName']}"
        ...
        지원 회사: {info["result"]["interview"]["corporateName"]}
        지원 직무: {info["result"]["interview"]["jobName"]}
        자기소개서: {cover_letter}

        이 정보를 바탕으로 본질적인 면접 질문을 {info["result"]["options"]["questionNumber"]}개 작성해줘.
        네가 생성한 질문 문자열을 line 별로 split하는 규칙 기반 알고리즘 수행 예정이야.
        그러니 면접 질문만 줄바꿈을 통해 답변해주고, "예, 알겠습니다"와 같은 답변은 절대 포함시키지 마.
        다시 한번 말할게. 질문만 줄바꿈을 통해 5개 생성해.
        """
        messages = [
        ...
        logging.info(f"🧮 Bedrock 토큰 사용량 - 입력: {input_tokens}, 출력: {output_tokens}")
        return output
